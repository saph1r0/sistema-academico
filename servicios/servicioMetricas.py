...
class ServicioMetricas:
    ...
    def _calcular_metricas_completas(self) -> MetricasSistema:
        ...
        metricas.fecha_calculo = timezone.now()
    ...
    def _calcular_metricas_usuarios(self, metricas: MetricasSistema):
        ...
        """
        # Contar usuarios por rol y estado (usando el nuevo modelo User)
        usuarios_stats = UsuarioModel.objects.aggregate(
            total_activos=Count('id', filter=Q(is_active=True)),
            total_inactivos=Count('id', filter=Q(is_active=False)),
            total_estudiantes=Count('id', filter=Q(role='student')),
            total_docentes=Count('id', filter=Q(role='teacher')),
            total_secretarias=Count('id', filter=Q(role='secretary')),
            total_administradores=Count('id', filter=Q(role='admin'))
        )
        
        # Usar campos que existen en MetricasSistema o agregar dinámicamente
        ...
        metricas.usuarios_activos_ultima_semana = 0  # Simplificado por ahora
    ...
    def _calcular_metricas_academicas(self, metricas: MetricasSistema):
        ...
        """
        # Contar matrículas activas (usando campos correctos del modelo)
        try:
            matriculas_stats = MatriculaModel.objects.aggregate(
                total_activas=Count('id', filter=Q(estado='ACTIVA')),
                total_cursos=Count('periodo_academico', distinct=True)
            )
            ...
            # Si hay error con las consultas, usar valores por defecto
            metricas.total_matriculas_activas = 0
            metricas.total_cursos_activos = 0
            metricas.nuevas_matriculas_mes = 0
    ...
    def _detectar_alertas_sistema(self, metricas: MetricasSistema):
        ...
        if hasattr(metricas, 'total_usuarios_inactivos') and hasattr(metricas, 'total_usuarios_activos'):
            if metricas.total_usuarios_inactivos > metricas.total_usuarios_activos:
                alerta = {
                    'tipo': 'warning',
                    'titulo': 'Alto número de usuarios inactivos',
                    'descripcion': f'Hay {metricas.total_usuarios_inactivos} usuarios inactivos vs {metricas.total_usuarios_activos} activos',
                    'prioridad': 'media',
        ...
        if metricas.promedio_asistencia_global < 70.0:
            alerta = {
                'tipo': 'error',
                'titulo': 'Promedio de asistencia bajo',
                'descripcion': f'El promedio de asistencia global es {metricas.promedio_asistencia_global}%, por debajo del 70%',
                'prioridad': 'alta',
                'fecha_creacion': timezone.now()
            }
            alertas.append(alerta)
        ...
        if hasattr(metricas, 'usuarios_activos_ultima_semana') and metricas.usuarios_activos_ultima_semana == 0:
            alerta = {
                'tipo': 'warning',
                'titulo': 'Sin actividad reciente',
                'descripcion': 'No hay usuarios activos en la última semana',
                'prioridad': 'media',
                'fecha_creacion': timezone.now()
            }
            alertas.append(alerta)
        ...
        if (hasattr(metricas, 'total_estudiantes') and hasattr(metricas, 'total_matriculas_activas') and
            metricas.total_estudiantes > 0 and metricas.total_matriculas_activas == 0):
            alerta = {
                'tipo': 'error',
                'titulo': 'Inconsistencia en datos',
                'descripcion': 'Hay estudiantes registrados pero sin matrículas activas',
                'prioridad': 'alta',
                'fecha_creacion': timezone.now()
            }
            alertas.append(alerta)
